...
export const CURRENCIES = ['RUB', 'USD', 'EUR', 'BRL']
export const COUNTS = [100, 1000, 5000, 10000, 25000, 50000, 100000]
...
export const getters = {
  ...
  platforms: (_, getters) => getters.greenToggle ? PLATFORMS : PLATFORMS.filter(platform => platform !== 'FAKE'),
  ...
  isFake: (_, getters) => getters.type === 'TYPE_TEST',
  isAuto: (_, getters) => getters.type?.includes('TYPE_AUTO'),
  isCustomComments: (_, getters) => getters.tag?.includes('COMMENTS_CUSTOM'),
  isInstagram: (_, getters) => getters.platform === 'INSTAGRAM',
  isTiktok: (_, getters) => getters.platform === 'TIKTOK',
  isYoutube: (_, getters) => getters.platform === 'YOUTUBE',
  ...
  quantityForCost: (_, getters) => (postQuantity, quantity = 0, quantityMin = 0, quantityMax = 0) => {
    if (getters.client === 'CLIENT_LK' && getters.isInstagram && getters.isAuto) {
...
export const actions = {
  ...
  async netServices ({ state, getters, commit }, { client = 'CLIENT_MAIN', platform = 'INSTAGRAM', prefetchedData = null } = {}) {
    ...
    if (prefetchedData) {
      ...
      response = await this.$http.services.tiny({
  ...
  async netCosts ({ state, getters, commit }, { client = 'CLIENT_MAIN', platform = 'INSTAGRAM', prefetchedCosts = null } = {}) {
    let response
    const cur = getters.localeCurrency
    const IS_MAIN = client === 'CLIENT_MAIN'
    ...
    if (!prefetchedCosts) {
      ...
      response = await this.$http.costs.tiny({
        ...IS_MAIN && { cur },
        labels: [client, 'VISIBLE'].join(' '),
        platform,
        counts
